...
from catalog_agent import _build_summary, fetch_chip_serials
...
async def catalog_write(state: _CatalogIn) -> _CatalogIO:
    ...
    if femb_serial:
        ...
        if chip_serials is None:
            mcp_warning = f"Django DB MCP server unreachable ({settings.django_mcp_url}); report written without chip serials."
    summary = _build_summary(findings, chip_serials)
